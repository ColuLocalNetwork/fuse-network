...
contract('VotingToChangeProxyAddress', async (accounts) => {
  ...
  beforeEach(async () => {
    ...
    let votingToChangeBlockRewardImpl = await VotingToChangeBlockReward.new()
    proxy = await EternalStorageProxy.new(proxyStorage.address, votingToChangeBlockRewardImpl.address)
    let votingToChangeBlockReward = await VotingToChangeBlockReward.at(proxy.address)
    await votingToChangeBlockReward.initialize(MIN_BALLOT_DURATION_SECONDS, GLOBAL_VALUES.BLOCK_REWARD)

    // VotingToChangeMinStake
    let votingToChangeMinStakeImpl = await VotingToChangeMinStake.new()
    proxy = await EternalStorageProxy.new(proxyStorage.address, votingToChangeMinStakeImpl.address)
    let votingToChangeMinStake = await VotingToChangeMinStake.at(proxy.address)
    await votingToChangeMinStake.initialize(MIN_BALLOT_DURATION_SECONDS, GLOBAL_VALUES.MIN_STAKE)

    // VotingToChangeMinThreshold
    let votingToChangeMinThresholdImpl = await VotingToChangeMinThreshold.new()
    proxy = await EternalStorageProxy.new(proxyStorage.address, votingToChangeMinThresholdImpl.address)
    let votingToChangeMinThreshold = await VotingToChangeMinThreshold.at(proxy.address)
    await votingToChangeMinThreshold.initialize(MIN_BALLOT_DURATION_SECONDS, GLOBAL_VALUES.VOTERS)
  ...
  describe('initialize', async () => {
    it('should be successful', async () => {
      await voting.initialize(MIN_BALLOT_DURATION_SECONDS).should.be.fulfilled
    ...
    it('should fail if min ballot duration is bigger than max ballot duration', async () => {
      ...
      await voting.initialize(maxBallotDuration.add(toBN(1))).should.be.rejectedWith(ERROR_MSG)
    })
  })
  ...
  describe('newBallot', async () => {
    ...
    it('should be successful', async () => {
      ...
      let {logs} = await voting.newBallot(VOTING_START_TIME, VOTING_END_TIME, contractType, proposedValue, 'description', {from: votingKeys[0]}).should.be.fulfilled
    ...
    it('should fail if not called by valid voting key', async () => {
      ...
      let proposedValue = RANDOM_ADDRESS
      let contractType = CONTRACT_TYPES.CONSENSUS
      await voting.newBallot(VOTING_START_TIME, VOTING_END_TIME, contractType, proposedValue, 'description', {from: nonVotingKey}).should.be.rejectedWith(ERROR_MSG)
    })
    it('should fail if times are invalid', async () => {
      ...
      VOTING_START_TIME = 0
      await voting.newBallot(VOTING_START_TIME, VOTING_END_TIME, contractType, proposedValue, 'description', {from: votingKeys[0]}).should.be.rejectedWith(ERROR_MSG)
      VOTING_END_TIME = 0
      await voting.newBallot(VOTING_START_TIME, VOTING_END_TIME, contractType, proposedValue, 'description', {from: votingKeys[0]}).should.be.rejectedWith(ERROR_MSG)
    ...
    it('should fail if creating ballot over the ballots limit', async () => {
      ...
      await voting.newBallot(VOTING_START_TIME, VOTING_END_TIME, contractType, proposedValue, 'description', {from: votingKeys[1]}).should.be.fulfilled
    })
  })
  ...
  describe('vote', async () => {
    ...
    beforeEach(async () => {
      ...
      await voting.newBallot(VOTING_START_TIME, VOTING_END_TIME, contractType, proposedValue, 'description', {from: votingKeys[0]}).should.be.fulfilled
    })
  ...
  describe('finalize', async () => {
    ...
    describe('should change all contract types implementations', async () => {
      ...
      it('ballots storage', async () => {
        let id = await voting.getNextBallotId()
        let proposedValue = RANDOM_ADDRESS
        let contractType = CONTRACT_TYPES.BALLOTS_STORAGE
        await voting.newBallot(VOTING_START_TIME, VOTING_END_TIME, contractType, proposedValue, 'description', {from: votingKeys[0]}).should.be.fulfilled
        await voting.setTime(VOTING_START_TIME)
        await voting.vote(id, ACTION_CHOICE.ACCEPT, {from: votingKeys[0]}).should.be.fulfilled
        await voting.vote(id, ACTION_CHOICE.ACCEPT, {from: votingKeys[1]}).should.be.fulfilled
        ...
        await voting.setTime(VOTING_END_TIME + 1)
        await voting.finalize(id, {from: votingKeys[0]}).should.be.fulfilled
        proposedValue.should.be.equal(await (await EternalStorageProxy.at(await proxyStorage.getBallotsStorage())).getImplementation())
      })
      ...
      it('voting to change block reward', async () => {
        let id = await voting.getNextBallotId()
        let proposedValue = RANDOM_ADDRESS
        let contractType = CONTRACT_TYPES.VOTING_TO_CHANGE_BLOCK_REWARD
        await voting.newBallot(VOTING_START_TIME, VOTING_END_TIME, contractType, proposedValue, 'description', {from: votingKeys[0]}).should.be.fulfilled
        await voting.setTime(VOTING_START_TIME)
        await voting.vote(id, ACTION_CHOICE.ACCEPT, {from: votingKeys[0]}).should.be.fulfilled
        await voting.vote(id, ACTION_CHOICE.ACCEPT, {from: votingKeys[1]}).should.be.fulfilled
        ...
        await voting.setTime(VOTING_END_TIME + 1)
        await voting.finalize(id, {from: votingKeys[0]}).should.be.fulfilled
        proposedValue.should.be.equal(await (await EternalStorageProxy.at(await proxyStorage.getVotingToChangeBlockReward())).getImplementation())
      })
      it('voting to change min stake', async () => {
        let id = await voting.getNextBallotId()
        let proposedValue = RANDOM_ADDRESS
        let contractType = CONTRACT_TYPES.VOTING_TO_CHANGE_MIN_STAKE
        await voting.newBallot(VOTING_START_TIME, VOTING_END_TIME, contractType, proposedValue, 'description', {from: votingKeys[0]}).should.be.fulfilled
        await voting.setTime(VOTING_START_TIME)
        await voting.vote(id, ACTION_CHOICE.ACCEPT, {from: votingKeys[0]}).should.be.fulfilled
        await voting.vote(id, ACTION_CHOICE.ACCEPT, {from: votingKeys[1]}).should.be.fulfilled
        ...
        await voting.setTime(VOTING_END_TIME + 1)
        await voting.finalize(id, {from: votingKeys[0]}).should.be.fulfilled
        proposedValue.should.be.equal(await (await EternalStorageProxy.at(await proxyStorage.getVotingToChangeMinStake())).getImplementation())
      })
      it('voting to change min threshold', async () => {
        let id = await voting.getNextBallotId()
        let proposedValue = RANDOM_ADDRESS
        let contractType = CONTRACT_TYPES.VOTING_TO_CHANGE_MIN_THRESHOLD
        await voting.newBallot(VOTING_START_TIME, VOTING_END_TIME, contractType, proposedValue, 'description', {from: votingKeys[0]}).should.be.fulfilled
        await voting.setTime(VOTING_START_TIME)
        await voting.vote(id, ACTION_CHOICE.ACCEPT, {from: votingKeys[0]}).should.be.fulfilled
        await voting.vote(id, ACTION_CHOICE.ACCEPT, {from: votingKeys[1]}).should.be.fulfilled
        ...
        await voting.setTime(VOTING_END_TIME + 1)
        await voting.finalize(id, {from: votingKeys[0]}).should.be.fulfilled
        proposedValue.should.be.equal(await (await EternalStorageProxy.at(await proxyStorage.getVotingToChangeMinThreshold())).getImplementation())
      })
      it('voting to change proxy', async () => {
        let id = await voting.getNextBallotId()
        let proposedValue = RANDOM_ADDRESS
        let contractType = CONTRACT_TYPES.VOTING_TO_CHANGE_PROXY
        await voting.newBallot(VOTING_START_TIME, VOTING_END_TIME, contractType, proposedValue, 'description', {from: votingKeys[0]}).should.be.fulfilled
        await voting.setTime(VOTING_START_TIME)
        await voting.vote(id, ACTION_CHOICE.ACCEPT, {from: votingKeys[0]}).should.be.fulfilled
        await voting.vote(id, ACTION_CHOICE.ACCEPT, {from: votingKeys[1]}).should.be.fulfilled
        ...
        await voting.setTime(VOTING_END_TIME + 1)
        await voting.finalize(id, {from: votingKeys[0]}).should.be.fulfilled
        proposedValue.should.be.equal(await (await EternalStorageProxy.at(await proxyStorage.getVotingToChangeProxy())).getImplementation())
      })
    })
  })
  ...
  describe('upgradeTo', async () => {
    ...
    it('should work after upgrade', async () => {
      ...
      await votingNew.initialize(MIN_BALLOT_DURATION_SECONDS).should.be.fulfilled
    ...
    it('should use same storage after upgrade', async () => {
      let newValue = MIN_BALLOT_DURATION_SECONDS + 60
      await voting.setMinBallotDurationMock(newValue)
      await proxy.setProxyStorageMock(proxyStorageStub)
      await proxy.upgradeTo(votingNew.address, {from: proxyStorageStub})
      votingNew = await Voting.at(proxy.address)
      toBN(newValue).should.be.bignumber.equal(await votingNew.getMinBallotDuration())
    })
  })
})
